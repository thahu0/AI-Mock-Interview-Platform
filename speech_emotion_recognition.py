...
class speechEmotionRecognition:
    ...
    def frame(self, y, win_step=64, win_size=128):
        ...
        if y.shape[2] < win_size:
            raise ValueError("Input signal length must be greater than or equal to window size.")

        # Number of frames
        nb_frames = 1 + int((y.shape[2] - win_size) / win_step)

        # Framing
        frames = np.zeros((y.shape[0], nb_frames, y.shape[1], win_size), dtype=np.float16)
    ...
    def predict_emotion_from_file(self, filename, chunk_step=16000, chunk_size=49100, predict_proba=False, sample_rate=16000):
        try:
            ...
            if predict_proba:
                ...
                predictions = np.argmax(self._model.predict(X), axis=1)
                predictions = [self._emotion.get(emotion) for emotion in predictions]

            # Clear session
            K.clear_session()

            # Calculate timestamps
            timestamp = np.concatenate([[chunk_size], np.ones((len(predictions) - 1)) * chunk_step]).cumsum()
    ...
    def prediction_to_csv(self, predictions, timestamps, filename, mode='w'):
        with open(filename, mode) as f:
            ...
            for ts, emotion in zip(timestamps, predictions):
                f.write(f"{ts},{emotion}\n")
