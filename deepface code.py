...
def gen():
    ...
    while end - start < max_time :
        ...
        # After reaching the end, write results to destination files
        if end - start > max_time - 1:
            try:
                ...
                with open("static/js/db/histo.txt", "a") as d:
                 ...
                 # Writing probabilities to CSV
                rows = zip(angry_0, disgust_1, fear_2, happy_3, sad_4, surprise_5, neutral_6)
                with open("static/js/db/prob.csv", "w", newline="") as d:
                    writer = csv.writer(d)
                    writer.writerows(rows)
        
                with open("static/js/db/prob_tot.csv", "a", newline="") as d:
                    writer = csv.writer(d)
                    writer.writerows(rows)
